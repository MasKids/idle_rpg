// 1회성 마이그레이션: 뽑기 비용을 50에서 30으로 더 내림("좀 퍼줘도 되니깐" —
// 043에서 동결한 50이 여전히 박하다는 피드백 반영). 레벨 무관 고정 유지.
//
// 실행: node scripts/migrations/044-gacha-cost-generous-30.mjs
// 재실행 방지 없음 — 항상 GachaTable.PullCostDiamond 전 행을 30으로 덮어쓴다.
...
async function main() {
  ...
  console.log(`[migration] GachaTable.PullCostDiamond 전 레벨 ${NEW_COST}로 인하 완료.`)
