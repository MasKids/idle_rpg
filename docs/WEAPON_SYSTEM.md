...
- **5종류**(v0.4.0에서 2종 추가): 검(Sword) / 창(Spear) / 활(Bow) / 도끼(Axe) /
  지팡이(Staff)
- **6등급**(v0.4.0에서 1개 추가): Normal / Rare / Epic / Unique / Legendary /
  Mythic
- **5단계(Tier 1~5)**: 등급 내에서의 세부 단계
- 종류 × 등급 × 단계 = **5 × 6 × 5 = 150개 아이템**(75개 → 150개로 확장)
- 종류별 특성(주 스탯):
  | 종류 | 주 스탯 |
  |---|---|
  | 검 | 공격력(ATK) |
  ...
  | 활 | 치명타 확률(CRIT) |
  | 도끼 | 치명타 피해(CRIT_DMG) |
  | 지팡이 | 존재력 획득량(EXIST_GAIN) |

기존 3종류가 6스탯 중 ATK/ASPD/CRIT를 이미 맡고 있어, 새 2종은 남은 전투
스탯(CRIT_DMG)과 비전투 스탯(EXIST_GAIN)으로 채워 5스탯 전부가 무기 특성으로
대표되게 했다. Mythic 등급 색은 기존 5색(회/하늘/보라/주황/금)과 겹치지 않는
로즈-레드(`--color-grade-mythic`)로 잡았다.
...
- "다음 단계"는 종류별로 30단계(6등급 × 5단계, v0.4.0에서 25→30)가 이어진
  하나의 사다리로 본다. 등급 내 단계(1→2→3→4→5)뿐 아니라 등급 경계도 자연스럽게
  이어진다. 예: 노말 5단계 5개 → 레어 1단계 1개.
- 사다리의 마지막 등급(현재 Mythic) 5단계는 사다리의 끝이므로 합성 불가.
...
- 뽑기 결과는 150개 무기 아이템(종류×등급×단계) 중 하나.
  - 종류(검/창/활/도끼/지팡이)는 균등 확률(1/5).
...
- 슬롯 수 = 존재력 트리 **30노드 해금마다 +1**(v0.4.0에서 250노드 확장에 맞춰
  10→30 간격으로 조정), 최대 8개. `RelicSlotTable`(30/60/90/120/150/180/210/240)
  기준으로 조회하며, 고정 공식이 아니다.
...
| RelicSlotTable | 38101 ~ 38108 | 8 (슬롯 1~8번째 해금 조건, v0.4.0에서 5→8, §6.6 참고) |
